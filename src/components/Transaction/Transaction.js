...
export default function Transactions({ data }) {
  ...
  let income = false;
  for (let i = 0; inputs && i < inputs.length; i += 1) {
    if (inputs[i].addresses && inputs[i].addresses[0] === ADDRESS) {
      income = true;
      break;
    }
  }
      ...
      <CardContent>
        ...
        <Grid item xs={12}>
          <Grid container justify="space-between">
            <Grid item xs={5} sm={5}>
              {inputs.map(item => <InputsItem key={item.script} item={item} />)}
            </Grid>
            <Grid item>
              <ArrowForwardIosIcon className={classes.icon} />
            </Grid>
            <Grid item xs={5} sm={5}>
              {outputs.map(item => item.value ? <OutputsItem key={item.script} item={item} /> : '')}
            </Grid>
          </Grid>
        </Grid>
      </CardContent>
